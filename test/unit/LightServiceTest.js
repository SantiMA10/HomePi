...
describe('LightService', function () {
    ...
    describe('work', function () {
        ...
        it('working:true, user:pepe, status:false', function () {
            chai_1.expect(service.work({ "working": true, "user": "pepe", "status": false, "config": "" })).to.deep.include({ "working": false, "user": process.env.SERVER_USER, "status": true, "config": "" });
        });
        it('working:true, user:pepe, status:true', function () {
            chai_1.expect(service.work({ "working": true, "user": "pepe", "status": true, "config": "" })).to.deep.include({ "working": false, "user": process.env.SERVER_USER, "status": false, "config": "" });
        });
    });
});
after(function (done) {
